...
The comparison against the previous implementation reads that implementation straight out of
git history, so nothing extra needs installing:

```bash
BASELINE_REF=8e38714 ./scripts/benchmark.sh   # the default; any ref works
```

`8e38714` is the last commit with the multi-pass `grep` engine. It is deliberately a commit rather
than the `v1.4.0` tag: that tag points at this release, so using it would compare the code with
itself.
...
*Reproduce:* `BASELINE_REF=8e38714 ./scripts/benchmark.sh`.
